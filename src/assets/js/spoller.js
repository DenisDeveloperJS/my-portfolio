...
if (spollerArray.length > 0) {
	...
	function setSpollerAction(e) {
		...
		if (el.hasAttribute("data-spoller") || el.closest("[data-spoller]")) {
			...
			const typeSpoller = spollerTitle.nextElementSibling.children[0];

			const arrangeText = block => {
				if (+window.innerWidth <= 991) {
					setTimeout(() => {
						spollerTitle.parentElement.scrollIntoView({
							behavior: "smooth",
							block: block,
							inline: "center",
						});
			...
			if (
				typeSpoller.children[0].classList.contains(
					"services-item__item_question"
				)
			) {
				arrangeText("center");
			} else {
				if (typeSpoller.classList.contains("services-item")) {
					arrangeText("center");
				}
			}
